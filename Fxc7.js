...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'TEL;type=CELL;type=VOICE;waid=557191467119:+55 7381-0440-96\n'
            + 'END:VCARD'

prefix = "!"
name = "~ 𝑨𝑵𝑰𝑴𝑬𝑿𝑿 𝑩𝑶𝑻"
rdaftar = "OBRIGADO POR SE REGISTRAR SEU LINDOR😏 "
rmenu = "PAE É VERIFICADO 😎🤚 "
botinfo = "PARA BOTS CONVIDADOS, DOE FIRST YAA:)"
limitt = 1000000000
memberLimit = 10000
ban = []
premium = ["557191467119@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Bem vindo @${num.split('@')[0]}\ncomporte-se para não toma ban *${mdata.subject}*`
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*ESPERA UM POUQUINHO AI...*',
				success: '*Pronto🖤..*',
				error: {
					stick: ' *falhou, ocorreu um erro ao converter a imagem em adesivo*',
					Iv: '*desculpe link inválido!!*'
				},
				only: {
					group: '*DESCULPE, ESTE COMANDO SÓ PODE SER USADO EM GRUPO!*',
					benned: '*desculpe, seu número foi banido, por favor contate o dono para abrir seu ban*',
					...
					premium: '*desculpe, este recurso é exclusivamente para usuários Premium!!*',
					userB: `*EIII ${pushname2} Você não está registrado no banco de dados. Digite \n${prefix}daftar*`,
					admin: '*desculpe, este comando só pode ser usado por administradores do grupo!*',
					Badmin: '*desculpe, este comando só pode ser usado quando o bot é administrado!*',
					publikG: `*desculpe, o bot foi privatizado pelo proprietário*\n*para mais detalhes digite*\n*${prefix}infobot*`
				}
			}

			const botNumber = frhan.user.jid
			const ownerNumber = ["557381044096@s.whatsapp.net"] // owner number ubah aja
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		setTimeout( () => {
		reply('Link de grupo detectado *${pushname2}* você vai ser chutado randomzinkakakaka')
		}, 0)
		}
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('Você já é amigo de Animexx hehe😏')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (!isPublic) return reply(mess.only.publikG)
				...
				case 'addprem':
					...
					premium = addpremium
					reply(`*Sucesso adicionado ${premium} Banco de dados de Usuário Premium * `)
					break
				case 'removeprem':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
				...
				var found = false
                    ...
                    for (let lmt of limidat) {
                        if (lmt.id === sender) {
                            ...
                            await reply(`*LIMITE SUA FICHA: ${limitCounts}*`)
                            found = true
                       }
                    }
                    if (found === false) {
                        let obj = { id : sender, limit : 1000 }
                        limit.push(obj);
                        fs.writeFileSync('./database/json/limit.json', JSON.stringify(limit, 100));
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('557191467119@s.whatsapp.net', options, text, {quoted: mek})
		...
		case 'happymod':
			...
			break
				default:
                  ...
                  reply(`Comando inválido *${prefix}${command}* !menu* para mais funções *${prefix}menu*`)
                  }
